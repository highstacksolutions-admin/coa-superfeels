...
their product, and sees that batch's independent laboratory report — the lab's own signed
PDF, shown in full on the page and ready to download. An admin panel manages batches and
their lab reports, products and customer enquiries, with analytics on what people are
looking up.
...
- **Report page** — the batch's lab report PDF, shown in full under the product name and
  batch code. Nothing is re-typed from the PDF, so the page can never disagree with the lab.
...
- **Batches** — a batch number and the lab's PDF, saved in one step (product optional;
  corrected or supporting PDFs can be added later). Products, Labs
...
npm run migrate
npm run seed:demo         # a few products, labs and batches with demo PDFs
...
`result_panels`, `results` and the lab/date/potency columns on `batches` are left over from
an earlier design where results were typed in from the PDF. Nothing writes to them any more;
migration 003 leaves them in place rather than drop data.
